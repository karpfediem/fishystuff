...
import {
  ...
  DEFAULT_ZONE_INFO_TAB,
  MAP_BRIDGE_SHARED_SIGNAL_BRANCHES,
  MAP_BRIDGE_SHARED_SIGNAL_WHITELIST,
  normalizeExpandedLayerIds,
...
import {
  renderSearchResults as renderSearchResultsPanel,
  renderSearchSelection as renderSearchSelectionPanel,
...
export function renderSearchSelection(elements, stateBundle, fishLookup) {
  return renderSearchSelectionPanel(elements, stateBundle, fishLookup, {
    ...
    formatZone,
    fishFilterTermMetadata: FISH_FILTER_TERM_METADATA,
  });
}

export function renderSearchResults(elements, matches, stateBundle) {
  return renderSearchResultsPanel(elements, matches, stateBundle, {
    setBooleanProperty,
    setTextContent,
    escapeHtml,
    fishFilterTermIconMarkup,
...
function renderPanel(
  ...
  layerUiState = DEFAULT_MAP_UI_SIGNAL_STATE.layers,
) {
  ...
  if (!activeOpacityInteraction && !activePointIconScaleInteraction) {
    renderLayerStack(
      ...
      {
        expandedLayerIds: new Set(normalizeExpandedLayerIds(layerUiState?.expandedLayerIds)),
...
function bindUi(shell, elements, options = {}) {
  ...
  function renderCurrentState(stateBundle = latestStateBundle || requestBridgeState(shell)) {
    ...
    const layerUiState = currentUiStateValue.layers;
    let bookmarkUi = {
  ...
  function reconcileUiStateFromSignals() {
    ...
    const nextSignature = JSON.stringify({
      ...
      layers: nextSignalUiState.layers,
  ...
  function toggleLayerSettings(layerId) {
    ...
    const currentLayers = currentUiState().layers;
    const expandedLayerIds = normalizeExpandedLayerIds(currentLayers?.expandedLayerIds);
    if (expandedLayerIds.includes(layerId)) {
      patchMapUiSignalState({
        layers: {
          expandedLayerIds: expandedLayerIds.filter((currentLayerId) => currentLayerId !== layerId),
        },
      });
      ...
      patchMapUiSignalState({
        layers: {
          expandedLayerIds: expandedLayerIds.concat(layerId),
        },
      });
    }
    renderCurrentState(getLatestStateBundle());
  ...
  async function resetMapUiToInitialState() {
    ...
    const resetLayerUiState = { expandedLayerIds: [] };
