...
calculator.breakdown.label.exp_share = EXP-Anteil
...
calculator.server.workspace.basics = Grundlagen
calculator.server.workspace.loadout = Ausrüstung
calculator.server.workspace.loot = Beute
calculator.server.workspace.trade = Handel
calculator.server.workspace.advanced = Erweitert
calculator.server.workspace.custom = Benutzerdefiniert
...
calculator.server.stat.expected_exp = Erwartete EXP
calculator.server.stat.exp_per_hour = EXP / Stunde
...
calculator.server.tab.exp_flow = EXP-Fluss
...
calculator.server.grade.prize = Preisfisch
calculator.server.grade.rare = Gelb
calculator.server.grade.high_quality = Blau
calculator.server.grade.general = Grün
calculator.server.grade.trash = Weiß
...
calculator.server.chart.no_exp_rows = Für diese Zone sind noch keine quellgestützten Fisch-EXP-Zeilen verfügbar.
...
calculator.server.chart.exp_flow_title = EXP-Fluss
calculator.server.chart.exp_flow_description = Jeder Fluss startet bei einer Fischgruppe, läuft durch quellgestützte Artenzeilen und wird dann zu EXP-gewichteten Gruppensummen zusammengeführt. Die linken Kennzahlen zeigen die Dropraten-Zusammensetzung, die rechten den EXP-Beitrag.
...
calculator.server.chart.group_exp_distribution_title = Gruppen-EXP-Verteilung
calculator.server.chart.group_exp_distribution_description = Erwarteter EXP-Anteil je Fischgruppe nach quellgestützten Fisch-EXP-Werten und aktiven EXP-Modifikatoren.
...
calculator.server.chart.aria.group_exp_distribution = Gruppen-EXP-Verteilung
calculator.server.chart.aria.loot_flow = Erwarteter Beutefluss von Gruppen zu Beutezeilen
calculator.server.chart.aria.exp_flow = Erwarteter EXP-Fluss von Gruppen zu Beutezeilen
...
calculator.server.group.tooltip.source_backed_share = DB-Anteil der Gruppe {$group}
...
calculator.breakdown.title.loot_total_totem_exp = Erwartete Totem-EXP ({$timespan})
calculator.breakdown.title.loot_totem_exp_per_hour = Totem-EXP / Stunde
calculator.breakdown.label.expected_totem_exp = Erwartete Totem-EXP
calculator.breakdown.label.expected_totem_exp_for_timespan = Erwartete Totem-EXP ({$timespan})
calculator.breakdown.label.group_expected_totem_exp = Erwartete Gruppen-Totem-EXP
calculator.breakdown.label.group_totem_exp = Gruppen-Totem-EXP
calculator.breakdown.label.totem_exp_per_hour = Totem-EXP / Stunde
calculator.breakdown.label.totem_exp_share = Totem-EXP-Anteil
calculator.breakdown.summary.loot_total_totem_exp = Erwartete Totem-EXP über die aktuelle Sitzungsdauer aus behaltenen quellgestützten oder per Stufenregel geschätzten Fischfängen.
calculator.breakdown.summary.loot_totem_exp_per_hour = Stündlicher Totem-EXP-Durchsatz aus behaltenen quellgestützten oder per Stufenregel geschätzten Fischfängen.
calculator.breakdown.formula.loot_total_totem_exp = Erwartete Totem-EXP = Σ(erwartete Gruppen-Totem-EXP).
calculator.breakdown.formula.loot_totem_exp_per_hour = Totem-EXP / Stunde = Erwartete Totem-EXP / Sitzungsstunden.
calculator.breakdown.detail.expected_totem_exp_selected_session = Erwartete Totem-EXP über die gewählte Sitzungsdauer.
calculator.breakdown.detail.expected_totem_exp_current_session = Erwartete Totem-EXP über die aktuelle Sitzungsdauer.
calculator.breakdown.detail.expected_hourly_totem_exp_throughput = Erwarteter stündlicher Totem-EXP-Durchsatz.
calculator.breakdown.detail.no_source_backed_loot_rows_contributing_expected_totem_exp = Derzeit tragen keine behaltenen Fischzeilen mit quellgestützter oder per Stufenregel geschätzter Totem-EXP bei.
calculator.breakdown.detail.group_expected_totem_exp_share = {$share} der erwarteten Totem-EXP

calculator.server.stat.expected_totem_exp = Erwartete Totem-EXP
calculator.server.stat.totem_exp_per_hour = Totem-EXP / Stunde
calculator.server.tab.totem_exp = Totem-EXP
calculator.server.tab.totem_exp_flow = Totem-EXP-Fluss
calculator.server.chart.no_totem_exp_rows = Für diese Zone tragen derzeit keine behaltenen Fischzeilen mit quellgestützter oder per Stufenregel geschätzter Totem-EXP bei.
calculator.server.chart.totem_exp_flow_title = Totem-EXP-Fluss
calculator.server.chart.totem_exp_flow_description = Jeder Fluss startet bei einer Fischgruppe, läuft durch behaltene Fischzeilen und wird dann zu Totem-EXP-gewichteten Gruppensummen zusammengeführt; die Herkunftsleiste markiert quellgestützte und per Stufenregel geschätzte Werte.
calculator.server.chart.group_totem_exp_distribution_title = Gruppen-Totem-EXP-Verteilung
calculator.server.chart.group_totem_exp_distribution_description = Erwarteter Totem-EXP-Anteil je Fischgruppe aus behaltenen quellgestützten oder per Stufenregel geschätzten Fischfängen.
calculator.server.chart.aria.group_totem_exp_distribution = Gruppen-Totem-EXP-Verteilung
calculator.server.chart.aria.totem_exp_flow = Erwarteter Totem-EXP-Fluss von Gruppen zu Beutezeilen

calculator.server.loot.breakdown.title.species_totem_exp_share = {$species} Totem-EXP-Anteil
calculator.server.loot.breakdown.title.group_totem_exp_share = {$group} Totem-EXP-Anteil
calculator.server.loot.breakdown.formula.species_totem_exp_share = Gegenstand erwartete Totem-EXP = Erwartete Fänge × Basis-Totem-EXP für behaltene Fische.; Totem-EXP-Anteil = Gegenstand erwartete Totem-EXP / Gesamte erwartete Gegenstands-Totem-EXP.
calculator.server.loot.breakdown.formula.group_totem_exp_share = Totem-EXP-Anteil = Erwartete Gruppen-Totem-EXP / Gesamte erwartete Gruppen-Totem-EXP.
calculator.server.loot.breakdown.label.base_totem_exp = Basis-Totem-EXP
calculator.server.loot.breakdown.label.item_expected_totem_exp = Gegenstand erwartete Totem-EXP
calculator.server.loot.breakdown.label.all_item_expected_totem_exp_total = Gesamte erwartete Gegenstands-Totem-EXP
calculator.server.loot.breakdown.label.all_group_expected_totem_exp_total = Gesamte erwartete Gruppen-Totem-EXP
calculator.server.loot.breakdown.summary.species_totem_exp_share.active = Erwarteter Totem-EXP-Beitrag dieser behaltenen Fischzeile.
calculator.server.loot.breakdown.summary.species_totem_exp_share.none = Diese Beutezeile trägt nach aktiver Quelle, Schätzung und Verwerf-Einstellungen derzeit keine erwartete Totem-EXP bei.
calculator.server.loot.breakdown.summary.group_totem_exp_share.active = Erwarteter Totem-EXP-Anteil aus behaltenen Fischzeilen, die dieser Gruppe zugeordnet sind.
calculator.server.loot.breakdown.summary.group_totem_exp_share.none = Diese Gruppe trägt nach aktiver Quelle, Schätzung und Verwerf-Einstellungen derzeit keine erwartete Totem-EXP bei.
calculator.server.loot.breakdown.detail.zero_exp_after_source = 0 EXP nach aktuellen Quelleinstellungen
calculator.server.loot.breakdown.detail.zero_totem_exp_after_source = 0 Totem-EXP nach aktuellen Quellen- und Schätzungseinstellungen
calculator.server.loot.breakdown.detail.base_totem_exp = Basis-Totem-EXP für einen behaltenen Fischfang.
calculator.server.loot.breakdown.detail.totem_exp_share.discarded = Verworfene Fische geben keine Totem-EXP.
calculator.server.loot.breakdown.detail.totem_exp_share.no_source_exp = Derzeit null, weil für diesen Gegenstand kein quellgestützter oder per Stufenregel geschätzter Totem-EXP-Wert verfügbar ist.
calculator.server.loot.breakdown.detail.totem_exp_share.expected_exp = Erwartete behaltene Fänge multipliziert mit Basis-Totem-EXP.
calculator.server.loot.breakdown.detail.all_item_expected_totem_exp_total = Nenner für die Totem-EXP-Fluss-Anteilsverteilung der Gegenstände.
calculator.server.loot.breakdown.detail.totem_exp_share.from_total = Berechnet aus erwarteter Gegenstands-Totem-EXP geteilt durch gesamte erwartete Totem-EXP.
calculator.server.loot.breakdown.detail.contributing_loot_totem_exp.unavailable = Derzeit tragen keine behaltenen Fischzeilen mit quellgestützter oder per Stufenregel geschätzter Totem-EXP zu dieser Gruppe bei.
calculator.server.loot.breakdown.detail.group_expected_totem_exp = Summe der behaltenen beitragenden Fischzeilen nach quellgestützten oder per Stufenregel geschätzten Totem-EXP-Werten.
calculator.server.loot.breakdown.detail.all_group_expected_totem_exp_total = Nenner für die Totem-EXP-Anteilsverteilung.
calculator.server.loot.breakdown.detail.group_totem_exp_share_from_total = Berechnet aus erwarteter Gruppen-Totem-EXP geteilt durch gesamte erwartete Totem-EXP.
calculator.server.provenance.metric.totem_exp = Totem-EXP
calculator.server.loot.totem_exp_source.source_backed = Quellgestützter %FishExpRate aus fish_size (flock) Encyclopedia_Table (2021).
calculator.server.loot.totem_exp_source.imputed_grade_rule = Quellgestützte Totem-EXP-Zeile fehlt; per Stufenregel geschätzt ({$grade}: {$exp} EXP). Grün verwendet konservativ 625, weil quellgestützte grüne Zeilen zwischen 625 und 1250 aufgeteilt sind.
calculator.server.loot.totem_exp_source.non_fish = Totem-EXP kann nur durch Fische erhalten werden.
calculator.server.loot.totem_exp_source.none = Kein quellgestützter oder per Stufenregel geschätzter Totem-EXP-Wert verfügbar.
calculator.server.loot.totem_exp_source.group_source_backed = Alle Totem-EXP-Werte in dieser Gruppe sind quellgestützte %FishExpRate-Zeilen.
calculator.server.loot.totem_exp_source.group_imputed = Alle Totem-EXP-Werte in dieser Gruppe sind per Stufenregel geschätzt.
calculator.server.loot.totem_exp_source.group_mixed = Totem-EXP kombiniert quellgestützte %FishExpRate-Zeilen und Stufenregel-Schätzungen ({$sourceBacked} quellgestützt, {$imputed} geschätzt).
