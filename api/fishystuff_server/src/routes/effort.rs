...
pub async fn effort_grid(
    ...
    payload: Result<Json<EffortGridRequest>, JsonRejection>,
) -> AppResult<Json<EffortGridResponse>> {
    ...
    if let Ok(mut cache) = state.cache.effort_grid.lock() {
        if let Some(cached) = cache.get(&cache_key) {
            let parsed: EffortGridResponse = serde_json::from_str(&cached).map_err(|err| {
                AppError::internal(format!("effort_grid cache decode failed: {err}"))
    ...
    let response = with_timeout(
        ...
        state.store.effort_grid(request),
    )
    .await
