...
impl DoltMySqlStore {
    pub(super) fn query_community_zone_support(
        ...
        let query = format!(
            "SELECT item_id, fish_name, support_status, claim_count \
             FROM community_zone_fish_support{as_of} \
             WHERE zone_rgb = ?"
        );

        let mut conn = self.pool.get_conn().map_err(db_unavailable)?;
        let rows: Vec<(i64, Option<String>, String, i64)> = match conn.exec(query, (zone_rgb_u32,))
        {
            Ok(rows) => rows,
        ...
        for (item_id, fish_name, support_status, claim_count) in rows {
