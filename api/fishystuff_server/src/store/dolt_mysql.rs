...
impl DoltMySqlStore {
    pub fn new(database_url: String, defaults: MetaDefaults) -> AppResult<Self> {
        let pool = Pool::new(database_url.as_str()).map_err(db_unavailable)?;
    ...
    fn load_events_with_zone_in_window(
        &self,
        ...
        let rows: Vec<(i64, i64, i64, i64, i64)> = conn
            .exec(
                queries::EVENTS_WITH_ZONE_SQL,
                (layer_revision_id, from_dt, to_dt),
            )
    ...
    fn compute_effort_grid(&self, params: &QueryParams) -> AppResult<EffortGridResponse> {
        ...
        let events = self.load_events_with_zone_in_window(
            &params.map_version,
            params.from_ts_utc,
            params.to_ts_utc,
        )?;
