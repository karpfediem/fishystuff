...
The production preflight is the aggregate local operator proof. It verifies the handoff summary, activation draft, admission evidence, edge handoff bundle, and dry-run host plan together, then runs the fast helper regressions unless `run_helper_tests=false` is passed. It intentionally does not apply the draft, install units, restart services, contact remote hosts, mutate DNS, or call cloud provider commands.
