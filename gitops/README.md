...
`just gitops-beta-first-service-set-plan` is the read-only runbook view for bringing up the first clean beta service set with the new toolset. It reports whether the beta handoff summary, admission evidence, activation draft, and proof chain already exist, refuses non-loopback API admission targets and non-beta artifacts, prints the guarded host actions in order, and emits a compact `operator_packet_*` section with the immediate next command for the current state. When runtime env files are missing, the packet includes beta-runtime SecretSpec readiness and embedded host-preflight status, then uses that preflight decision as the effective next action instead of blindly saying to write env files. The proof/apply/edge phases route through read-only packet commands before any guarded mutation command, so the operator sees exact proof and unit hashes first. `just gitops-beta-first-service-set-packet` runs the same checks and prints only that compact packet plus no-mutation safety flags for operator handoff. Runtime-env readiness now leads to `start_or_verify_beta_services` before admission evidence, so the plan no longer implies admission can happen before the guarded beta Dolt/API service start or an explicit operator verification. It deliberately does not run mgmt, install units, restart services, SSH, mutate DNS, or provision hosts.
