...
`just gitops-beta-first-service-set-plan` is the read-only runbook view for bringing up the first clean beta service set with the new toolset. It reports whether the beta handoff summary, admission evidence, activation draft, and proof chain already exist, refuses non-loopback API admission targets and non-beta artifacts, and prints the guarded host actions in order. It deliberately does not run mgmt, install units, restart services, SSH, mutate DNS, or provision hosts.
