...
`just gitops-beta-write-activation-admission-evidence` and `just gitops-beta-activation-draft` are beta defaults over the shared admission and activation-draft checks. They refuse non-beta handoff summaries. `just gitops-beta-admission-packet` and `just gitops-beta-activation-draft-packet` are read-only packet views for that boundary: the first validates or requests loopback admission evidence, and the second validates or requests the serving activation draft before operator proof. A beta activation draft still requires explicit admission evidence and a retained rollback release; the current no-retained beta handoff is intentionally candidate-only.
