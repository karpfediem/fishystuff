...
`edge-service-bundle-beta-gitops-handoff` is the first clean beta edge shape for a distinct beta service set. It uses `fishystuff-beta-edge.service`, beta hostnames, `/var/lib/fishystuff/gitops-beta/served/beta/{site,cdn}`, `/run/fishystuff/beta-edge/tls`, and the beta loopback API upstream `127.0.0.1:18192`. `just gitops-beta-edge-handoff-bundle` validates the exact local bundle and rejects production hostnames, production served roots, production TLS paths, production edge dependencies, legacy `/srv/fishystuff`, and fixed `/nix/store` serving roots. See `BETA.md` for the current beta service-set boundary.
