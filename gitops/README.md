...
`just gitops-beta-operator-proof`, `just gitops-check-beta-operator-proof`, `just gitops-beta-operator-proof-packet`, `just gitops-beta-served-proof`, `just gitops-beta-served-proof-packet`, and `just gitops-beta-proof-index` are beta-default wrappers over the environment-parametric proof plumbing. They write/check beta-named proof artifacts, keep beta state under `/var/lib/fishystuff/gitops-beta` and `/run/fishystuff/gitops-beta`, and refuse non-beta handoff summaries or proof environments. The operator-proof packet is read-only and prints the guarded beta apply command only after the selected proof is current. The served-proof packet is also read-only and moves the operator from checked proof to served state verification to served proof/index without applying or restarting anything.
