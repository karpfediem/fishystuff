...
just gitops-beta-write-runtime-env
just gitops-beta-check-runtime-env
...
just gitops-beta-remote-host-preflight
just gitops-beta-remote-host-bootstrap
just gitops-beta-remote-host-test
just gitops-beta-remote-install-nix
...
just gitops-beta-remote-start-edge
just gitops-beta-remote-start-edge-test
just gitops-beta-remote-install-tls-resident-packet
just gitops-beta-remote-install-tls-resident-packet-test
just gitops-beta-remote-tls-resident-status-packet
just gitops-beta-remote-tls-resident-status-packet-test
just gitops-beta-remote-install-tls-resident
just gitops-beta-remote-install-tls-resident-test
...
just gitops-beta-tls-resident-install-packet
just gitops-beta-tls-resident-install-packet-test
just gitops-beta-tls-resident-status-packet
just gitops-beta-tls-resident-status-packet-test
just gitops-beta-install-tls-resident
just gitops-beta-install-tls-resident-test
...
just gitops-beta-install-service
...
just gitops-production-host-handoff-plan
...
just gitops-production-operator-proof admission_file=/tmp/fishystuff-production-admission.json
just gitops-check-production-operator-proof
just gitops-production-served-proof
...
`just gitops-beta-write-activation-admission-evidence` and `just gitops-beta-activation-draft` are beta defaults over the shared admission and activation-draft checks. They refuse non-beta handoff summaries. `just gitops-beta-admission-packet` and `just gitops-beta-activation-draft-packet` are read-only packet views for that boundary: the first validates or requests loopback admission evidence before handing off to the second, and the second validates or requests the serving activation draft before handing off to operator proof readiness. A beta activation draft still requires explicit admission evidence and a retained rollback release; the current no-retained beta handoff is intentionally candidate-only.

`just gitops-beta-host-handoff-plan` is the beta dry-run host handoff review. It checks the beta activation tuple against the beta edge handoff bundle and prints beta service paths, TLS paths, API upstream, refusal conditions, the guarded beta apply command, and the guarded beta edge install command. It does not apply the draft, install units, or restart services.

`just gitops-beta-operator-proof`, `just gitops-check-beta-operator-proof`, `just gitops-beta-operator-proof-packet`, `just gitops-beta-served-proof`, `just gitops-beta-served-proof-packet`, `just gitops-beta-edge-install-packet`, and `just gitops-beta-proof-index` are beta-default wrappers over the environment-parametric proof plumbing. They write/check beta-named proof artifacts, keep beta state under `/var/lib/fishystuff/gitops-beta` and `/run/fishystuff/gitops-beta`, and refuse non-beta handoff summaries or proof environments. The operator-proof packet is read-only and prints the guarded beta apply command only after the selected proof is current. The served-proof packet is also read-only and moves the operator from checked proof to served state verification to served proof/index without applying or restarting anything. The edge-install packet is read-only and exposes the reviewed served-proof and beta edge unit hashes required by the final guarded edge install.

`just gitops-beta-apply-activation-draft` is the separate beta local apply gate. It requires `FISHYSTUFF_GITOPS_ENABLE_BETA_APPLY=1`, `FISHYSTUFF_GITOPS_ENABLE_LOCAL_APPLY=1`, `FISHYSTUFF_GITOPS_BETA_APPLY_OPERATOR_PROOF_SHA256` from `just gitops-check-beta-operator-proof`, and the expected beta resident hostname; it refuses production summaries/proofs before invoking mgmt. The MCL path helpers route beta `local-apply` state to `/var/lib/fishystuff/gitops-beta` and `/run/fishystuff/gitops-beta`, while production keeps `/var/lib/fishystuff/gitops` and `/run/fishystuff/gitops`.

`just gitops-beta-edge-install-packet` reports whether the beta proof chain is complete and, when it is, validates the beta edge bundle, prints current host context, and prints the exact env-gated `just gitops-beta-install-edge` command. `just gitops-beta-install-edge` is the beta-only edge install/restart gate. It refuses to run without `FISHYSTUFF_GITOPS_ENABLE_BETA_EDGE_INSTALL=1`, `FISHYSTUFF_GITOPS_ENABLE_BETA_EDGE_RESTART=1`, `FISHYSTUFF_GITOPS_BETA_EDGE_SERVED_PROOF_SHA256=<checked beta served proof hash>`, `FISHYSTUFF_GITOPS_BETA_EDGE_UNIT_SHA256=<checked beta edge unit hash>`, and the expected beta resident hostname. It re-checks the complete beta proof index and beta edge bundle, installs only `fishystuff-beta-edge.service`, reloads systemd, restarts that beta unit, and checks it is active. The regression uses fake `install`, `hostname`, and `systemctl` commands so the guard can be tested without host mutation.
...
```json
{
  ...
  "tls": {
    "edge": {
      ...
      "reload_service": "fishystuff-edge",
...
The next local-only bridge is `just gitops-production-activation-draft`. It reads the verified handoff summary plus an explicit admission evidence JSON file and writes `data/gitops/production-activation.draft.desired.json` by default. The draft changes the selected environment to `mode: local-apply`, `serve: true`, `transition.kind: activate`, and `admission_probe.kind: api_meta`; it does not run mgmt apply or mutate production. Admission evidence must use schema `fishystuff.gitops.activation-admission.v1` and match the exact handoff summary SHA-256, desired-state SHA-256, release ID, release identity, Dolt commit, API meta response, a representative DB-backed probe, and a site/CDN runtime probe. The API upstream in that evidence must be the host-local loopback candidate API, not the public API URL, so Caddy, route state, and admission all agree on the same selected instance. `just gitops-write-activation-admission-evidence` writes that evidence from observed probe JSON. `just gitops-check-activation-draft` re-checks an existing draft against the same handoff and evidence before any later apply path may consume it. `just gitops-review-activation-draft` is the read-only operator view of that exact tuple. These admission and activation checks are environment-parametric from the handoff summary; beta uses the `gitops-beta-*` wrappers for safer defaults. `just gitops-production-operator-proof` and `just gitops-check-production-operator-proof` attach that tuple to host inventory, preflight, and handoff-plan evidence; the beta equivalents are `just gitops-beta-operator-proof` and `just gitops-check-beta-operator-proof`. `just gitops-apply-activation-draft` is the guarded production local mgmt consumer: it requires `FISHYSTUFF_GITOPS_ENABLE_PRODUCTION_APPLY=1`, `FISHYSTUFF_GITOPS_ENABLE_LOCAL_APPLY=1`, a checked operator proof file, and `FISHYSTUFF_GITOPS_APPLY_OPERATOR_PROOF_SHA256` from `gitops-check-production-operator-proof` before running a local one-shot reconciliation. `just gitops-beta-apply-activation-draft` is the separate beta gate using `FISHYSTUFF_GITOPS_ENABLE_BETA_APPLY=1`, `FISHYSTUFF_GITOPS_BETA_APPLY_OPERATOR_PROOF_SHA256` from `gitops-check-beta-operator-proof`, and the expected beta resident hostname. `just gitops-verify-activation-served` is the read-only served-state verifier after reconciliation, and `just gitops-beta-verify-activation-served` is the beta-default wrapper that refuses non-beta summaries and checks `/var/lib/fishystuff/gitops-beta` plus `/run/fishystuff/gitops-beta`. `just gitops-production-served-proof` wraps the production verifier and writes the post-activation audit artifact tying served state back to the checked operator proof; `just gitops-beta-served-proof` and `just gitops-beta-proof-index` provide the same beta-shaped post-reconciliation proof chain. `just gitops-beta-install-edge` is the final beta local-host edge gate after that proof chain is complete; it requires the expected beta resident hostname plus independently reviewed served-proof and unit hashes before installing/restarting only the beta edge unit.
...
`edge-service-bundle-production-gitops-handoff` is the production edge bundle shape for that same handoff. It keeps the production public hostnames, points the site/CDN roots at `/var/lib/fishystuff/gitops/served/production/{site,cdn}`, uses a stable local candidate API upstream, and disables activation-created content roots so the service bundle cannot overwrite GitOps-managed symlinks. It is checked by `.#checks.x86_64-linux.edge-service-bundle-production-gitops-handoff`; `just gitops-production-edge-handoff-bundle` builds or validates the exact local bundle, rejects legacy `/srv/fishystuff`, fixed `/nix/store` serving roots, beta hostnames, or a wrong candidate API upstream, cross-checks `bundle.json` with the systemd unit run/reload commands and artifact symlink targets, and runs `caddy validate` against the generated Caddyfile with temporary placeholder TLS credentials. `just gitops-production-host-inventory` prints a read-only view of current local served/status/rollback/admission/route paths, served symlinks, installed edge unit provenance, edge bundle validation proof, and TLS certificate metadata. `just gitops-production-host-handoff-plan` combines the checked activation draft, admission evidence, and edge bundle metadata into a dry-run host-local handoff plan without installing units or restarting services; its output separates read-only readiness checks, guarded host actions, and post-handoff read-only checks. `just gitops-production-preflight` runs the handoff-summary check, activation draft check/review, edge bundle check, host handoff plan, and by default the fast helper regression suite as one local-only operator proof; pass `run_helper_tests=false` for fixture tests or a faster artifact-only re-check. When `served_state_dir=/var/lib/fishystuff/gitops` or an explicit `rollback_set_path=...` is supplied, it also derives retained rollback releases from the served rollback-set documents and requires them to match the handoff summary exactly. `just gitops-production-operator-proof` runs inventory, preflight, and host handoff plan together and writes a timestamped JSON proof artifact under `data/gitops/` by default. `just gitops-check-production-operator-proof` verifies a stored proof artifact's schema, age, local command outputs, no-mutation flags, and current hashes for the activation draft, handoff summary, and admission evidence before anything consumes it.

`api-service-bundle-beta-gitops-handoff` and `dolt-service-bundle-beta-gitops-handoff` are the first clean beta API/Dolt service shapes for a distinct beta service set. They use `fishystuff-beta-api.service`, `fishystuff-beta-dolt.service`, beta-only runtime env files under `/var/lib/fishystuff/gitops-beta`, and a beta Dolt state directory under `/var/lib/fishystuff/beta-dolt`. The API bundle deliberately splits operator-owned runtime config at `/var/lib/fishystuff/gitops-beta/api/runtime.env` from GitOps-owned release identity at `/var/lib/fishystuff/gitops-beta/api/beta.env`, so local reconciliation can update release facts without overwriting database/CORS/CDN settings. `edge-service-bundle-beta-gitops-handoff` uses `fishystuff-beta-edge.service`, beta hostnames, `/var/lib/fishystuff/gitops-beta/served/beta/{site,cdn}`, `/var/lib/fishystuff/gitops-beta/tls/live`, and the beta loopback API upstream `127.0.0.1:18192`. `just gitops-beta-service-bundles-test` validates the API/Dolt/edge bundle checks. `just gitops-beta-check-service-bundle` validates the real beta API or Dolt bundle from shell before host install; `just gitops-beta-write-runtime-env`, `just gitops-beta-write-runtime-env-secretspec`, `just gitops-beta-check-runtime-env`, `just gitops-beta-runtime-env-packet`, `just gitops-beta-runtime-env-host-preflight`, and the runtime-env tests cover the host-local runtime env boundary and reject shared or production material, with the API DB DSN scoped to the narrow SecretSpec `beta-runtime` profile when desired and reported by the runtime-env packet alongside embedded preflight status/action before guarded writes; real `/var/lib/fishystuff/gitops-beta` env writes require the expected beta resident hostname while `/tmp` fixture writes remain local-testable. `just gitops-beta-host-bootstrap-plan` prints the read-only fresh-host contract, including the current hostname and expected beta resident hostname match, before any user/dir/env/unit work and hands off to the service-start and admission packets; `just gitops-beta-host-bootstrap-apply` is the explicit opt-in local executor for the beta directories and beta Dolt user/group only, and it refuses to run unless the local hostname matches that expected beta resident hostname; `just gitops-beta-service-start-plan` and `just gitops-beta-service-start-packet` compose the checked env files and API/Dolt bundles into a read-only start review with unit hashes and the guarded Dolt-before-API start command, resolving `auto` bundles from the beta handoff summary when it exists, and the packet reports missing runtime env as pending instead of a direct failure; `just gitops-beta-start-services` is the explicit opt-in sequence gate that rechecks that same handoff-backed plan, requires the expected beta resident hostname, and enforces beta Dolt before beta API; `just gitops-beta-install-service` is the guarded beta API/Dolt local install/restart gate with service-specific opt-ins, reviewed unit hashes, and the same hostname guard. `just gitops-beta-edge-handoff-bundle` validates the exact local edge bundle and rejects production hostnames, production served roots, production TLS paths, production edge dependencies, legacy `/srv/fishystuff`, and fixed `/nix/store` serving roots. `just gitops-beta-current-desired` then assembles those beta artifacts into a validate-mode desired-state file without applying it, and `just gitops-beta-current-handoff` writes the local handoff proof for that exact tuple. `just gitops-beta-observe-admission`, `just gitops-beta-admission-packet`, and `just gitops-beta-activation-draft-packet` capture or verify loopback API, local site/CDN runtime evidence, and activation-draft readiness for that tuple before operator proof. `just gitops-beta-operator-proof-packet`, `just gitops-beta-served-proof-packet`, `just gitops-beta-edge-install-packet`, `just gitops-beta-verify-activation-served`, `just gitops-beta-operator-proof`, `just gitops-beta-served-proof`, `just gitops-beta-proof-index`, `just gitops-beta-apply-activation-draft`, `just gitops-beta-install-service`, and `just gitops-beta-install-edge` are the beta proof/apply/install chain wrappers; they keep the beta state/run roots explicit, emit beta-named proof schemas, require beta-specific opt-ins, require the expected beta resident hostname before local mutation, and refuse production summaries, proof environments, or service unit names. See `BETA.md` for the current beta service-set boundary.

`just gitops-beta-deploy-credentials-packet` is the read-only local `beta-deploy` SecretSpec readiness check for that first beta service set. It reports presence of deploy tokens and beta SSH key material, validates the SSH private/public key pair by deriving the public key, prints only the public key fingerprint, and points to `just deploy-key-boundary-check` before any real host use. `FISHYSTUFF_GITOPS_ENABLE_BETA_DEPLOY_KEY_GENERATE=1 just gitops-beta-deploy-key-ensure` can generate and store missing beta SSH key material locally through SecretSpec, but it deliberately does not upload keys, SSH to hosts, mutate cloud providers, or touch system services.

`just gitops-beta-host-provision-plan` is the no-mutation provision/select packet for the first clean beta resident host. It records the intended Hetzner host shape and beta deploy SSH key name, carries the credential packet status, warns that public beta DNS must not be used for a new host until the operator confirms it points at that host, and emits only manual confirmation steps. It does not emit or run `hcloud`, SSH, DNS, deploy, or local-host mutation commands.

`just gitops-beta-host-selection-packet public_ipv4=<new-beta-public-ip>` is the no-mutation handoff from provision/select review to fresh-host bootstrap. It binds the operator-confirmed public IPv4 into `FISHYSTUFF_BETA_RESIDENT_TARGET=root@<ip>` prefixes for subsequent packets, so follow-up checks do not rely on `beta.fishystuff.fish` before DNS has intentionally moved.

`FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy secretspec run --profile beta-deploy -- just gitops-beta-remote-host-preflight target=root@<new-beta-public-ip>` is the read-only SSH preflight for that selected host. It refuses DNS targets, checks hostname/OS/systemd/Nix presence plus beta user/group/directory state, and reports no mutation. `FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_BOOTSTRAP=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_DIRECTORIES=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_USER_GROUPS=1 secretspec run --profile beta-deploy -- just gitops-beta-remote-host-bootstrap target=root@<new-beta-public-ip>` is the guarded SSH mutation for only the beta-local user/group and directory scaffold. `FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_NIX_INSTALL=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_NIX_APT_PREREQS=1 secretspec run --profile beta-deploy -- just gitops-beta-remote-install-nix target=root@<new-beta-public-ip>` installs multi-user Nix on that beta host when preflight reports Nix is missing; closure transfer, env writes, unit installs, service starts, DNS, and cloud mutation remain separate steps.
...
`just gitops-beta-remote-install-tls-resident-packet target=root@<new-beta-public-ip>` is the read-only review packet for the remote mgmt ACME resident bridge. It validates the beta desired/unit shape, computes the desired/unit/token hashes, checks that the target is a fresh root IPv4 target, and emits the exact guarded `gitops-beta-remote-install-tls-resident` command without printing the Cloudflare token. `FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_TLS_RESIDENT_INSTALL=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_TLS_RESIDENT_CLOSURE_COPY=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_TLS_RESIDENT_RESTART=1 FISHYSTUFF_GITOPS_BETA_REMOTE_TLS_RESIDENT_TARGET=root@<new-beta-public-ip> FISHYSTUFF_GITOPS_BETA_TLS_DESIRED_SHA256=<checked-desired-sha256> FISHYSTUFF_GITOPS_BETA_TLS_RESIDENT_UNIT_SHA256=<checked-unit-sha256> FISHYSTUFF_GITOPS_BETA_TLS_CLOUDFLARE_TOKEN_SHA256=<checked-token-sha256> secretspec run --profile beta-deploy -- just gitops-beta-remote-install-tls-resident target=root@<new-beta-public-ip>` is the guarded remote bridge for the mgmt ACME resident. It copies only the reviewed beta TLS desired state, reviewed resident unit, and beta Cloudflare token material, verifies hashes on the target, installs under `/var/lib/fishystuff/gitops-beta` plus `fishystuff-beta-tls-reconciler.service`, and starts only that resident unit. The generated unit uses a finite convergence timeout by default and systemd CPU/IO/memory/task controls so a bad mgmt loop cannot starve SSH or Caddy. `just gitops-beta-remote-tls-resident-status-packet target=root@<new-beta-public-ip>` is the read-only SSH status packet for the installed resident, reporting unit state, desired/unit/token file metadata, and certificate SAN/expiry/key-match facts without reading token contents.
...
`just gitops-beta-tls-desired`, `just gitops-beta-tls-reconcile-packet`, and `just gitops-beta-reconcile-tls` are the first beta TLS path that goes through mgmt ACME instead of the manual certificate-copy bridge. The generator defaults to Let's Encrypt staging and writes only a local desired-state file. The packet validates beta-only domains, beta TLS paths, the beta ACME request namespace, beta edge reload notification, and beta-deploy Cloudflare token readiness without applying anything. The reconcile command is host-local mutation and requires beta/local apply opt-ins, the expected beta resident hostname, `CLOUDFLARE_API_TOKEN` from `beta-deploy`, and an extra production ACME opt-in if `ca=production`. `just gitops-beta-tls-resident-unit` writes the durable systemd unit for the same graph, using the pinned `.#mgmt-gitops` binary and packaged `.#gitops-src` graph while keeping desired state under `/var/lib/fishystuff/gitops-beta` and loading the beta-only Cloudflare token through systemd `LoadCredential`; the default unit run is finite and resource-limited. The resident uses `gitops/tls-main.mcl` so TLS reconciliation compiles only the TLS desired-state slice instead of the release/environment graph. `just gitops-beta-tls-resident-install-packet` is the read-only review packet for that resident unit: it validates the desired/unit shape, computes reviewed hashes, reports the beta hostname gate, and does not print the token value. By default, the token source is `env:CLOUDFLARE_API_TOKEN`, so run the packet/install commands under `secretspec run --profile beta-deploy`. `just gitops-beta-tls-resident-status-packet` is the read-only host-local status view for the installed resident: it reports unit state, desired/unit/token file metadata, certificate SAN/expiry/key-match checks, and the beta hostname context without printing the Cloudflare token. `just gitops-beta-install-tls-resident` is the guarded local beta-host install/start gate for that unit and requires those reviewed hashes before enabling `fishystuff-beta-tls-reconciler.service`.
...
`FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_HETZNER_CREATE=1 FISHYSTUFF_GITOPS_BETA_HETZNER_CREATE_SERVER_NAME=site-nbg1-beta-v2 secretspec run --profile beta-deploy -- just gitops-beta-hetzner-create-host` is the explicit replacement-host create gate. It uses the Hetzner API because `hcloud` is not required, checks inventory first, creates only a distinct beta replacement server, applies beta labels, and sets the in-VM hostname to the resident name expected by later host-local guards.

`FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_HETZNER_RETIRE=1 FISHYSTUFF_GITOPS_BETA_HETZNER_RETIRE_SERVER_NAME=<old-beta-server-name> FISHYSTUFF_GITOPS_BETA_HETZNER_RETIRE_SERVER_ID=<old-beta-server-id> FISHYSTUFF_GITOPS_BETA_HETZNER_RETIRE_SERVER_IPV4=<old-beta-ip> FISHYSTUFF_GITOPS_BETA_HETZNER_ACTIVE_SERVER_IPV4=<new-beta-ip> secretspec run --profile beta-deploy -- just gitops-beta-hetzner-retire-host retire_server_name=<old-beta-server-name> retire_server_id=<old-beta-server-id> retire_server_ipv4=<old-beta-ip> active_server_name=<replacement-server-name> active_server_ipv4=<new-beta-ip>` is the explicit old-host retire gate. It deletes only the exact acknowledged old beta server id after proving a distinct active beta server exists, refuses production profiles and GitOps-service-set-labelled targets, and polls Hetzner inventory until the old id is absent.

`just gitops-beta-first-service-set-plan` is the read-only runbook view for bringing up the first clean beta service set with the new toolset. It reports whether the beta handoff summary, admission evidence, activation draft, and proof chain already exist, refuses non-loopback API admission targets and non-beta artifacts, runs the local beta deploy-authority and deploy-credential checks, prints the guarded host actions in order, and emits a compact `operator_packet_*` section with the immediate next command, deploy credential readiness, credential-authority state, and current host context for the current state. When runtime env files are missing, the packet includes beta-runtime SecretSpec readiness and embedded host-preflight status, then uses that preflight decision as the effective next action instead of blindly saying to write env files. The proof/apply/edge phases route through read-only packet commands before any guarded mutation command, so the operator sees exact proof hashes, unit hashes, deploy credential readiness, credential-authority state, and host context first. `just gitops-beta-first-service-set-packet` runs the same checks and prints only that compact packet plus no-mutation safety flags for operator handoff. Runtime-env readiness now leads to `start_or_verify_beta_services` before admission evidence, so the plan no longer implies admission can happen before the guarded beta Dolt/API service start or an explicit operator verification. It deliberately does not run mgmt, install units, restart services, SSH, mutate DNS, or provision hosts.
