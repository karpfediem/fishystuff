...
`just gitops-beta-operator-proof`, `just gitops-check-beta-operator-proof`, `just gitops-beta-served-proof`, and `just gitops-beta-proof-index` are beta-default wrappers over the environment-parametric proof plumbing. They write/check beta-named proof artifacts, keep beta state under `/var/lib/fishystuff/gitops-beta` and `/run/fishystuff/gitops-beta`, and refuse non-beta handoff summaries or proof environments. The served proof/index remain post-reconciliation audit steps; they do not apply or restart anything.
