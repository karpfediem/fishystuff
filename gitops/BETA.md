...
FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_EDGE_START=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_EDGE_CLOSURE_COPY=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_EDGE_SERVED_LINKS=1 FISHYSTUFF_GITOPS_BETA_REMOTE_EDGE_TLS_MODE=existing FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_EDGE_EXISTING_TLS=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_EDGE_INSTALL=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_EDGE_RESTART=1 FISHYSTUFF_GITOPS_BETA_REMOTE_EDGE_TARGET=root@<new-beta-public-ip> FISHYSTUFF_GITOPS_BETA_EDGE_UNIT_SHA256=<checked-beta-edge-unit-sha256> secretspec run --profile beta-deploy -- just gitops-beta-remote-start-edge target=root@<new-beta-public-ip>
secretspec run --profile beta-deploy -- just gitops-beta-remote-install-tls-resident-packet target=root@<new-beta-public-ip>
FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_TLS_RESIDENT_INSTALL=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_TLS_RESIDENT_CLOSURE_COPY=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_TLS_RESIDENT_RESTART=1 FISHYSTUFF_GITOPS_BETA_REMOTE_TLS_RESIDENT_TARGET=root@<new-beta-public-ip> FISHYSTUFF_GITOPS_BETA_TLS_DESIRED_SHA256=<checked-desired-sha256> FISHYSTUFF_GITOPS_BETA_TLS_RESIDENT_UNIT_SHA256=<checked-unit-sha256> FISHYSTUFF_GITOPS_BETA_TLS_CLOUDFLARE_TOKEN_SHA256=<checked-token-sha256> secretspec run --profile beta-deploy -- just gitops-beta-remote-install-tls-resident target=root@<new-beta-public-ip>
FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy secretspec run --profile beta-deploy -- just gitops-beta-remote-tls-resident-status-packet target=root@<new-beta-public-ip>
...
FISHYSTUFF_GITOPS_BETA_ACME_CONTACT_EMAIL=<email> just gitops-beta-tls-desired
secretspec run --profile beta-deploy -- just gitops-beta-tls-reconcile-packet
FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_TLS_APPLY=1 FISHYSTUFF_GITOPS_ENABLE_LOCAL_APPLY=1 secretspec run --profile beta-deploy -- just gitops-beta-reconcile-tls
just gitops-beta-tls-resident-unit
secretspec run --profile beta-deploy -- just gitops-beta-tls-resident-install-packet
just gitops-beta-tls-resident-status-packet
FISHYSTUFF_GITOPS_ENABLE_BETA_TLS_RESIDENT_INSTALL=1 FISHYSTUFF_GITOPS_ENABLE_BETA_TLS_RESIDENT_RESTART=1 FISHYSTUFF_GITOPS_BETA_TLS_DESIRED_SHA256=<checked-desired-sha256> FISHYSTUFF_GITOPS_BETA_TLS_RESIDENT_UNIT_SHA256=<checked-unit-sha256> FISHYSTUFF_GITOPS_BETA_TLS_CLOUDFLARE_TOKEN_SHA256=<checked-token-sha256> secretspec run --profile beta-deploy -- just gitops-beta-install-tls-resident
...
FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_HETZNER_CREATE=1 FISHYSTUFF_GITOPS_BETA_HETZNER_CREATE_SERVER_NAME=site-nbg1-beta-v2 secretspec run --profile beta-deploy -- just gitops-beta-hetzner-create-host
just gitops-beta-hetzner-create-host-test
...
`FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy secretspec run --profile beta-deploy -- just gitops-beta-remote-host-preflight target=root@<new-beta-public-ip>` is the read-only SSH preflight for a fresh beta host. It requires the beta deploy SSH key, refuses DNS targets, checks the remote hostname, OS, systemd, Nix/Nix daemon presence, beta user/group, and beta directories, and reports no host mutation. `FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_BOOTSTRAP=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_DIRECTORIES=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_USER_GROUPS=1 secretspec run --profile beta-deploy -- just gitops-beta-remote-host-bootstrap target=root@<new-beta-public-ip>` is the guarded remote bootstrap for only the beta Dolt user/group and beta directories.

`FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_NIX_INSTALL=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_NIX_APT_PREREQS=1 secretspec run --profile beta-deploy -- just gitops-beta-remote-install-nix target=root@<new-beta-public-ip>` installs the official multi-user Nix daemon setup on the fresh Debian beta host after the preflight reports `install_remote_nix`. It may run `apt-get update`, install `ca-certificates`, `curl`, and `xz-utils`, download `https://nixos.org/nix/install`, and run it with `--daemon --yes`. It does not write FishyStuff runtime env files, install FishyStuff units, start FishyStuff services, deploy releases, mutate Hetzner, mutate Cloudflare, or touch DNS.

`FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_CLOSURE_COPY=1 FISHYSTUFF_GITOPS_BETA_REMOTE_CLOSURE_TARGET=root@<new-beta-public-ip> secretspec run --profile beta-deploy -- just gitops-beta-copy-handoff-closures target=root@<new-beta-public-ip>` copies only the exact API, site, CDN runtime, and Dolt service closure paths from the checked beta handoff summary to the selected host. It refuses production profiles, DNS targets, the previous beta IP, non-beta summaries, unchecked closure summaries, missing local closure paths, and target/acknowledgement mismatches.

`FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_RUNTIME_ENV_COPY=1 FISHYSTUFF_GITOPS_BETA_REMOTE_RUNTIME_ENV_TARGET=root@<new-beta-public-ip> secretspec run --profile beta-deploy -- just gitops-beta-copy-runtime-env target=root@<new-beta-public-ip>` generates the beta API runtime env through the narrow `beta-runtime` SecretSpec profile, validates the API and Dolt env files locally, then installs them at `/var/lib/fishystuff/gitops-beta/api/runtime.env` and `/var/lib/fishystuff/gitops-beta/dolt/beta.env` on the selected host. It does not install units, start services, deploy releases, mutate Hetzner, mutate Cloudflare, or touch DNS.

`FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_DOLT_REF_MATERIALIZE=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_DOLT_STOP_SERVICES=1 FISHYSTUFF_GITOPS_BETA_REMOTE_DOLT_REF_TARGET=root@<new-beta-public-ip> secretspec run --profile beta-deploy -- just gitops-beta-remote-materialize-dolt-ref target=root@<new-beta-public-ip>` updates the fresh host's beta Dolt repo from `origin/beta` and pins the checked GitOps release ref from the handoff summary. It stops only `fishystuff-beta-api.service` and `fishystuff-beta-dolt.service`, runs `dolt fetch origin beta`, resets local `beta` to `origin/beta`, force-points `fishystuff/gitops-beta/<release>` at the exact checked commit, and leaves service restart to the separate start gate. This step exists because the API reads the checked release through `FISHYSTUFF_DEFAULT_DOLT_REF`, not through a mutable plain `beta` branch.

`FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_SERVICE_START=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_DOLT_INSTALL=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_DOLT_RESTART=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_API_INSTALL=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_API_RESTART=1 FISHYSTUFF_GITOPS_BETA_REMOTE_SERVICE_TARGET=root@<new-beta-public-ip> FISHYSTUFF_GITOPS_BETA_DOLT_UNIT_SHA256=<checked-dolt-unit-sha256> FISHYSTUFF_GITOPS_BETA_API_UNIT_SHA256=<checked-api-unit-sha256> secretspec run --profile beta-deploy -- just gitops-beta-remote-start-services target=root@<new-beta-public-ip>` is the guarded remote bridge from copied closures/env files to running beta Dolt/API units. It re-checks the exact beta handoff summary and beta service bundles locally, refuses DNS targets and the previous beta IP, checks the reviewed unit hashes, installs only `fishystuff-beta-dolt.service` and `fishystuff-beta-api.service` on the selected host, starts Dolt before API, and waits for loopback Dolt SQL plus API `/api/v1/meta` to report the selected release and Dolt commit. It does not mutate DNS, Hetzner, Cloudflare, production, edge routing, or the old beta host.

`FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_EDGE_START=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_EDGE_CLOSURE_COPY=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_EDGE_SERVED_LINKS=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_EDGE_PLACEHOLDER_TLS=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_EDGE_INSTALL=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_EDGE_RESTART=1 FISHYSTUFF_GITOPS_BETA_REMOTE_EDGE_TARGET=root@<new-beta-public-ip> FISHYSTUFF_GITOPS_BETA_EDGE_UNIT_SHA256=<checked-beta-edge-unit-sha256> secretspec run --profile beta-deploy -- just gitops-beta-remote-start-edge target=root@<new-beta-public-ip>` is the guarded remote bridge from a running loopback beta API to beta HTTPS origin smoke. It copies only the checked beta edge closure, makes `/var/lib/fishystuff/gitops-beta` traversable but not listable for the dynamic Caddy user, publishes `/var/lib/fishystuff/gitops-beta/served/beta/{site,cdn}` as symlinks to the exact handoff site/CDN closures, installs only `fishystuff-beta-edge.service`, and probes beta site/API/CDN through `curl -k --resolve ...:127.0.0.1` on the remote host. The `PLACEHOLDER_TLS` opt-in is intentionally not browser-trusted and exists only to test the new origin before beta DNS or mgmt-issued DNS01 certificates are moved into place. For an already trusted beta host, set `FISHYSTUFF_GITOPS_BETA_REMOTE_EDGE_TLS_MODE=existing` with `FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_EDGE_EXISTING_TLS=1`; that preserves and validates `/var/lib/fishystuff/gitops-beta/tls/live/{fullchain.pem,privkey.pem}` and uses trusted loopback HTTPS checks. It does not mutate DNS, Hetzner, Cloudflare, production, or the old beta host.

`just gitops-beta-remote-install-tls-resident-packet target=root@<new-beta-public-ip>` is the read-only review packet for the durable mgmt ACME resident bridge. It validates the beta desired/unit shape, computes the desired/unit/token hashes, checks the fresh root IPv4 target, and emits the exact guarded remote install command without printing the Cloudflare token. `FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_TLS_RESIDENT_INSTALL=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_TLS_RESIDENT_CLOSURE_COPY=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_TLS_RESIDENT_RESTART=1 FISHYSTUFF_GITOPS_BETA_REMOTE_TLS_RESIDENT_TARGET=root@<new-beta-public-ip> FISHYSTUFF_GITOPS_BETA_TLS_DESIRED_SHA256=<checked-desired-sha256> FISHYSTUFF_GITOPS_BETA_TLS_RESIDENT_UNIT_SHA256=<checked-unit-sha256> FISHYSTUFF_GITOPS_BETA_TLS_CLOUDFLARE_TOKEN_SHA256=<checked-token-sha256> secretspec run --profile beta-deploy -- just gitops-beta-remote-install-tls-resident target=root@<new-beta-public-ip>` is the guarded remote bridge for the durable mgmt ACME resident. It re-checks the reviewed hashes locally, copies only the beta TLS desired state, resident unit, and beta Cloudflare token material to temporary remote paths, verifies the same hashes on the host, installs under `/var/lib/fishystuff/gitops-beta`, and starts only `fishystuff-beta-tls-reconciler.service`. The generated unit defaults to a finite convergence run and includes systemd CPU, IO, memory, and task limits so a bad mgmt loop cannot starve SSH or Caddy. `just gitops-beta-remote-tls-resident-status-packet target=root@<new-beta-public-ip>` is the read-only SSH status packet for the resident unit, installed desired/unit/token files, and beta certificate SAN/expiry/key-match facts. These commands refuse production profiles, the previous beta IP, non-root/non-IPv4 targets, DNS mutation outside beta, Hetzner mutation, production services, and the old beta host.

`FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_EDGE_TLS_INSTALL=1 FISHYSTUFF_GITOPS_ENABLE_BETA_REMOTE_EDGE_RESTART=1 FISHYSTUFF_GITOPS_BETA_REMOTE_EDGE_TLS_TARGET=root@<new-beta-public-ip> FISHYSTUFF_GITOPS_BETA_EDGE_TLS_FULLCHAIN_SHA256=<checked-fullchain-sha256> FISHYSTUFF_GITOPS_BETA_EDGE_TLS_PRIVKEY_SHA256=<checked-privkey-sha256> secretspec run --profile beta-deploy -- just gitops-beta-remote-install-edge-tls target=root@<new-beta-public-ip> fullchain=<local-fullchain.pem> privkey=<local-privkey.pem>` installs operator-supplied beta-only TLS material after checking that the certificate covers beta/API/CDN/telemetry beta hostnames, is not near expiry, matches the private key, and matches reviewed fullchain/private-key hashes. It restarts only `fishystuff-beta-edge.service` and repeats trusted loopback HTTPS smokes without `-k`. It is the bridge for either today's old-beta certificate copy or the later mgmt `acme:certificate` materialization output; it does not mutate DNS, Hetzner, Cloudflare, production, or the old beta host.

`just gitops-beta-tls-desired` writes a beta-only local-apply desired-state document for the mgmt ACME graph. It defaults to Let's Encrypt staging and refuses production ACME desired state unless `FISHYSTUFF_GITOPS_ENABLE_BETA_TLS_PRODUCTION_DESIRED=1` is set. `just gitops-beta-tls-reconcile-packet` is the read-only operator packet: it validates that desired state uses only beta domains, `/var/lib/fishystuff/gitops-beta/tls/live`, the beta ACME request namespace, beta edge reload notification, and the pinned `.#mgmt-gitops` package, then reports beta-deploy Cloudflare token readiness and the exact guarded reconcile command. `just gitops-beta-reconcile-tls` is the local beta-host mutation gate that runs the mgmt ACME graph; it requires `FISHYSTUFF_GITOPS_ENABLE_BETA_TLS_APPLY=1`, `FISHYSTUFF_GITOPS_ENABLE_LOCAL_APPLY=1`, the expected beta resident hostname, and `CLOUDFLARE_API_TOKEN` from the beta-deploy SecretSpec profile. `just gitops-beta-tls-resident-unit` writes the durable systemd unit for the same graph, with `.#mgmt-gitops`, `.#gitops-src`, beta-only desired paths, a finite default convergence run, resource controls, and a beta-only Cloudflare token credential loaded through systemd `LoadCredential`. The resident runs `gitops/tls-main.mcl`, not the full GitOps graph, so certificate renewal does not pay the unrelated release/environment type-unification cost. `just gitops-beta-tls-resident-install-packet` computes the reviewed hashes for the desired state, unit, and `CLOUDFLARE_API_TOKEN` environment value without printing the token, reports the beta hostname gate, and emits the exact guarded install command only when ready. `just gitops-beta-tls-resident-status-packet` is read-only and reports the installed resident unit state, desired/unit/token file metadata, beta certificate SAN/expiry/key-match checks, and hostname context without printing token contents. `just gitops-beta-install-tls-resident` is the hostname-gated local install/start gate for that unit; it requires reviewed hashes for the desired state, unit, and token, installs only beta TLS resident files, enables/restarts only `fishystuff-beta-tls-reconciler.service`, and refuses production SecretSpec profiles. Production ACME additionally requires `FISHYSTUFF_GITOPS_ENABLE_BETA_TLS_PRODUCTION_ACME=1`, so staging remains the default first path.

`FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_DNS_CUTOVER=1 FISHYSTUFF_GITOPS_BETA_DNS_TARGET_IPV4=<new-beta-public-ip> secretspec run --profile beta-deploy -- just gitops-beta-cloudflare-dns-cutover target_ipv4=<new-beta-public-ip>` updates only these existing Cloudflare A records: `beta.fishystuff.fish`, `api.beta.fishystuff.fish`, `cdn.beta.fishystuff.fish`, and `telemetry.beta.fishystuff.fish`. It refuses the previous beta IP, production profiles, non-`fishystuff.fish` zones, missing target acknowledgement, duplicate/missing records, and any hostname outside that beta set. It does not mutate Hetzner, SSH hosts, production DNS records, or production services.
...
`FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_HETZNER_CREATE=1 FISHYSTUFF_GITOPS_BETA_HETZNER_CREATE_SERVER_NAME=site-nbg1-beta-v2 secretspec run --profile beta-deploy -- just gitops-beta-hetzner-create-host` is the guarded infrastructure mutation for creating the replacement beta host through the Hetzner API. It refuses production profiles, refuses the old beta server name, requires the replacement name acknowledgement, checks inventory first, sets cloud-init hostname to `site-nbg1-beta`, and emits the next host-selection/preflight commands.

`FISHYSTUFF_OPERATOR_SECRETSPEC_PROFILE=beta-deploy FISHYSTUFF_GITOPS_ENABLE_BETA_HETZNER_RETIRE=1 FISHYSTUFF_GITOPS_BETA_HETZNER_RETIRE_SERVER_NAME=<old-beta-server-name> FISHYSTUFF_GITOPS_BETA_HETZNER_RETIRE_SERVER_ID=<old-beta-server-id> FISHYSTUFF_GITOPS_BETA_HETZNER_RETIRE_SERVER_IPV4=<old-beta-ip> FISHYSTUFF_GITOPS_BETA_HETZNER_ACTIVE_SERVER_IPV4=<new-beta-ip> secretspec run --profile beta-deploy -- just gitops-beta-hetzner-retire-host retire_server_name=<old-beta-server-name> retire_server_id=<old-beta-server-id> retire_server_ipv4=<old-beta-ip> active_server_name=<replacement-server-name> active_server_ipv4=<new-beta-ip>` is the guarded infrastructure mutation for deleting the old beta Hetzner server after beta DNS and proof have moved to the replacement. It refuses production profiles, requires exact server name/id/IP acknowledgements, requires a distinct active beta server with `fishystuff.deployment=beta`, refuses to delete a server labelled as the GitOps service set, and polls inventory until the retired server is absent.
