...
`gitops-served-candidate-vm` boots a local NixOS VM with `serve: true` in `vm-test` mode. It checks fixture admission, candidate state, served status, and the VM-local active selection file under `/var/lib/fishystuff/gitops-test/active/local-test.json`. It still asserts that no real FishyStuff services or deployment directories are touched.
