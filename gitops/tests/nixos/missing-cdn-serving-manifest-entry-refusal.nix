...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    machine.succeed("env FISHYSTUFF_GITOPS_STATE_FILE=${desiredState} ${mgmtPackage}/bin/mgmt run --hostname vm-single-host --tmp-prefix --no-pgp --client-urls=http://127.0.0.1:2379 --server-urls=http://127.0.0.1:2380 --advertise-client-urls=http://127.0.0.1:2379 --advertise-server-urls=http://127.0.0.1:2380 --converged-timeout=-1 lang ${gitopsSrc}/main.mcl >/tmp/fishystuff-gitops-missing-cdn-serving-manifest-entry-refusal.log 2>&1 & echo $! >/tmp/fishystuff-gitops-missing-cdn-serving-manifest-entry-refusal.pid")
    machine.wait_until_succeeds("grep -F 'serving artifact admission requires CDN serving manifest to list selected runtime module and wasm' /tmp/fishystuff-gitops-missing-cdn-serving-manifest-entry-refusal.log")
    machine.succeed("kill $(cat /tmp/fishystuff-gitops-missing-cdn-serving-manifest-entry-refusal.pid) || true")
