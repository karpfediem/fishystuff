...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    wait_for_gitops_command(f"jq -e '.desired_generation == 63 and .release_id == \"second-local-apply-http-release\" and .release_identity == \"${expectedSecondReleaseIdentity}\" and .phase == \"served\" and .admission_state == \"passed_fixture\" and .served == true and .retained_release_ids == [\"local-apply-http-release\"]' {status}")
