...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    machine.wait_until_succeeds(f"jq -e '.desired_generation == 30 and .release_id == \"candidate-release\" and .phase == \"served\" and .transition_kind == \"activate\" and .rollback_from_release == \"\" and .rollback_to_release == \"\"' {status}")
    ...
    machine.wait_until_succeeds(f"jq -e '.desired_generation == 31 and .release_id == \"previous-release\" and .phase == \"served\" and .transition_kind == \"rollback\" and .rollback_from_release == \"candidate-release\" and .rollback_to_release == \"previous-release\" and .rollback_reason == \"operator-requested rollback test\"' {status}")
