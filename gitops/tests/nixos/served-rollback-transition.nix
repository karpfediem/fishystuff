...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    machine.wait_until_succeeds(f"${fishystuffDeployPackage}/bin/fishystuff_deploy gitops check-served --status {status} --active {active} --rollback-set {rollback_set} --rollback {rollback} --environment local-test --host vm-single-host --release-id candidate-release")
    machine.wait_until_succeeds(f"${fishystuffDeployPackage}/bin/fishystuff_deploy gitops summary-served --status {status} --active {active} --rollback-set {rollback_set} --rollback {rollback} --environment local-test --host vm-single-host --release-id candidate-release | grep -Fx 'served_release: candidate-release'")
    machine.wait_until_succeeds(f"${fishystuffDeployPackage}/bin/fishystuff_deploy gitops summary-served --status {status} --active {active} --rollback-set {rollback_set} --rollback {rollback} --environment local-test --host vm-single-host --release-id candidate-release | grep -Fx 'retained_rollback_releases: previous-release'")
    ...
    machine.wait_until_succeeds(f"${fishystuffDeployPackage}/bin/fishystuff_deploy gitops check-served --status {status} --active {active} --rollback-set {rollback_set} --rollback {rollback} --environment local-test --host vm-single-host --release-id previous-release")
    machine.wait_until_succeeds(f"${fishystuffDeployPackage}/bin/fishystuff_deploy gitops summary-served --status {status} --active {active} --rollback-set {rollback_set} --rollback {rollback} --environment local-test --host vm-single-host --release-id previous-release | grep -Fx 'served_release: previous-release'")
    machine.wait_until_succeeds(f"${fishystuffDeployPackage}/bin/fishystuff_deploy gitops summary-served --status {status} --active {active} --rollback-set {rollback_set} --rollback {rollback} --environment local-test --host vm-single-host --release-id previous-release | grep -Fx 'retained_rollback_releases: candidate-release'")
