...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    active_roots_ready = f"/run/fishystuff/gitops/roots/production-{release_id}.json"
    previous_roots_ready = f"/run/fishystuff/gitops/roots/production-{previous_release_id}.json"
    ...
    wait_for_gitops_file(active_roots_ready)
    wait_for_gitops_file(previous_roots_ready)
    ...
    machine.succeed(f"jq -e '.desired_generation == 4 and .environment == \"production\" and .host == \"production-single-host\" and .release_id == \"{release_id}\" and .release_identity == \"{expected_release_identity}\" and .root_count == 4 and .require_nix_gcroot == true and .roots_ready == true and .state == \"roots_ready\" and ([.roots[] | select(.name == \"api\" and .root_path == \"/nix/var/nix/gcroots/fishystuff/gitops/{release_id}/api\" and .store_path == \"${apiArtifact}\" and .symlink_ready == true and .nix_gcroot_ready == true)] | length) == 1 and ([.roots[] | select(.name == \"cdn_runtime\" and .root_path == \"/nix/var/nix/gcroots/fishystuff/gitops/{release_id}/cdn-runtime\" and .store_path == \"${cdnRuntimeArtifact}\" and .symlink_ready == true and .nix_gcroot_ready == true)] | length) == 1' {active_roots_ready}")
    machine.succeed(f"jq -e '.desired_generation == 4 and .environment == \"production\" and .host == \"production-single-host\" and .release_id == \"{previous_release_id}\" and .root_count == 4 and .require_nix_gcroot == true and .roots_ready == true and .state == \"roots_ready\" and ([.roots[] | select(.name == \"api\" and .root_path == \"/nix/var/nix/gcroots/fishystuff/gitops/{previous_release_id}/api\" and .store_path == \"${previousApiArtifact}\" and .symlink_ready == true and .nix_gcroot_ready == true)] | length) == 1 and ([.roots[] | select(.name == \"cdn_runtime\" and .root_path == \"/nix/var/nix/gcroots/fishystuff/gitops/{previous_release_id}/cdn-runtime\" and .store_path == \"${previousCdnRuntimeArtifact}\" and .symlink_ready == true and .nix_gcroot_ready == true)] | length) == 1' {previous_roots_ready}")
