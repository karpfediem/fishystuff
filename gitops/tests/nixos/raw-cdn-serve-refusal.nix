...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    machine.succeed("env FISHYSTUFF_GITOPS_STATE_FILE=${desiredState} ${mgmtPackage}/bin/mgmt run --hostname vm-single-host --tmp-prefix --no-pgp --client-urls=http://127.0.0.1:2379 --server-urls=http://127.0.0.1:2380 --advertise-client-urls=http://127.0.0.1:2379 --advertise-server-urls=http://127.0.0.1:2380 --converged-timeout=-1 lang ${gitopsSrc}/main.mcl >/tmp/fishystuff-gitops-raw-cdn-refusal.log 2>&1 & echo $! >/tmp/fishystuff-gitops-raw-cdn-refusal.pid")
    machine.wait_until_succeeds("grep -F cdn-serving-manifest.json /tmp/fishystuff-gitops-raw-cdn-refusal.log")
    machine.succeed("kill $(cat /tmp/fishystuff-gitops-raw-cdn-refusal.pid)")
