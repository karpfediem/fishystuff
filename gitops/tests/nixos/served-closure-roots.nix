...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    machine.succeed(f"jq -e '.desired_generation == 40 and .release_id == \"active-release\" and .release_identity == \"${expectedReleaseIdentity}\" and .site_root == \"/var/lib/fishystuff/gitops-test/served/site\" and .cdn_root == \"/var/lib/fishystuff/gitops-test/served/cdn\" and .served == true and .state == \"selected_local_route\"' {route}")
    ...
    machine.succeed("test \"$(readlink /var/lib/fishystuff/gitops-test/served/site)\" = \"${activeSiteArtifact}\"")
    machine.succeed("test \"$(readlink /var/lib/fishystuff/gitops-test/served/cdn)\" = \"${activeCdnServingRoot}\"")
