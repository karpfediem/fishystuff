...
pub use crate::field::{DiscreteFieldRows, FieldRgbaChunk, ZoneLookupRows};
...
impl ZoneMask {
    ...
    pub fn to_lookup_rows(&self) -> Result<ZoneLookupRows> {
        let mut rgbs = Vec::with_capacity((self.width as usize) * (self.height as usize));
        for pixel in self.data.chunks_exact(3) {
            rgbs.push(pack_rgb_u32(pixel[0], pixel[1], pixel[2]));
        }
        ZoneLookupRows::from_u32_grid(self.width, self.height, &rgbs)
    }
}
