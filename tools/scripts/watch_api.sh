...
devenv_notify_status "starting API watcher on ${API_BIND_HOST}:${API_PORT}"
devenv_run_with_http_ready \
  "http://${API_BIND_HOST}:${API_PORT}/api/v1/meta" \
  "API server ready on ${API_BIND_HOST}:${API_PORT}; watching for changes" \
  watchexec -r \
    -w api \
    -w lib \
    -w Cargo.toml \
