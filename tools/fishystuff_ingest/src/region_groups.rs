...
use fishystuff_core::gamecommondata::{
    load_original_regiongroupinfo_rows, load_original_regioninfo_rows,
};
...
pub fn load_region_group_inputs(
    ...
    let regioninfo_rows = load_original_regioninfo_rows(regioninfo_bss_path)?;
    let regiongroupinfo_rows = load_original_regiongroupinfo_rows(regiongroupinfo_bss_path)?;

    let mut accum: BTreeMap<u32, GroupAccumulator> = BTreeMap::new();
    let mut region_rows: Vec<RegionGroupRegionRow> = Vec::new();

    for row in regioninfo_rows {
        ...
        region_rows.push(RegionGroupRegionRow {
            ...
            region_id: row.key,
            trade_origin_region: (row.tradeoriginregion != 0).then_some(row.tradeoriginregion),
    ...
    for row in regiongroupinfo_rows {
        if row.key == 0 {
            continue;
        }
        let group = accum.entry(row.key).or_default();
        if group.graph_world_x.is_none() && (row.graphx != 0.0 || row.graphz != 0.0) {
