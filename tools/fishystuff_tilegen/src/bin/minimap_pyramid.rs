...
struct Args {
    /// Directory containing level-0 source tiles named rader_<x>_<y>.png
    #[arg(long)]
    input_dir: PathBuf,
    ...
    #[arg(long, default_value = "/images/tiles/minimap/v1")]
    root_url: String,
    /// When true, larger Y tile indices are visually higher in the map.
...
fn main() -> Result<()> {
    ...
    if level0.is_empty() {
        bail!("no minimap tiles found in {}", args.input_dir.display());
    }
    ...
    for (z, coords) in levels.into_iter().enumerate() {
        ...
        manifest_levels.push(LevelManifest {
            ...
            path: "{z}/rader_{x}_{y}.png".to_string(),
...
fn build_next_level(
    ...
    for ((px, py), children) in parents {
        ...
        for (cx, cy, qx, qy) in children {
            ...
            let draw_qy = draw_quadrant_y(qy, y_flip);
            overlay(
                &mut canvas,
                &child,
                (qx * tile_px) as i64,
                (draw_qy * tile_px) as i64,
            );
            has_child = true;
        }
        if !has_child {
...
fn tile_name(x: i32, y: i32) -> String {
    format!("rader_{}_{}.png", x, y)
}

fn parse_tile_name(name: &str) -> Option<(i32, i32)> {
    let stem = name.strip_suffix(".png")?;
    let stem = stem.strip_prefix("rader_")?;
...
fn draw_quadrant_y(qy: u32, y_flip: bool) -> u32 {
    if y_flip {
        1 - qy
    } else {
        qy
...
#[cfg(test)]
mod tests {
    use super::{draw_quadrant_y, tile_parent_quadrant};

    #[test]
    fn euclidean_parent_quadrant_handles_negative_coords() {
        assert_eq!(tile_parent_quadrant(-2, -2), (-1, -1, 0, 0));
        assert_eq!(tile_parent_quadrant(-1, -2), (-1, -1, 1, 0));
        assert_eq!(tile_parent_quadrant(-2, -1), (-1, -1, 0, 1));
        assert_eq!(tile_parent_quadrant(-1, -1), (-1, -1, 1, 1));
    }

    #[test]
    fn y_flip_places_higher_tile_indices_on_top() {
        assert_eq!(draw_quadrant_y(0, false), 0);
        assert_eq!(draw_quadrant_y(1, false), 1);
        assert_eq!(draw_quadrant_y(0, true), 1);
        assert_eq!(draw_quadrant_y(1, true), 0);
    }
}
