...
use crate::plugins::api::{FishCatalog, FishFilterState, MapDisplayState, PatchFilterState};
...
pub(in crate::bridge::host) fn apply_browser_input_state(
    ...
    if !bridge.is_changed() && !fish.is_changed() && !layer_registry.is_changed() {
        return;
    }

    layer_runtime.sync_to_registry(&layer_registry);
    filters::apply_display_flags(&bridge.input, &mut display_state, &mut debug_layers);
    filters::apply_fish_filters(&bridge.input, &fish, &mut fish_filter);
