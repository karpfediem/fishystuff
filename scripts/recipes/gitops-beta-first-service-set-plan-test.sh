...
grep -F "operator_packet_runtime_env_host_preflight_current_hostname=operator-dev" "${fixture_root}/ready.stdout" >/dev/null
grep -F "operator_packet_runtime_env_host_preflight_expected_hostname=site-nbg1-beta" "${fixture_root}/ready.stdout" >/dev/null
grep -F "operator_packet_runtime_env_host_preflight_expected_hostname_match=false" "${fixture_root}/ready.stdout" >/dev/null
grep -F "operator_packet_runtime_env_host_preflight_resident_target=root@beta.fishystuff.fish" "${fixture_root}/ready.stdout" >/dev/null
