...
bash scripts/recipes/gitops-beta-tls-resident-unit.sh \
  ...
  "${mgmt_dir}/mgmt" \
  "$gitops_dir" \
  /var/lib/fishystuff/gitops-beta/secrets/acme.env \
...
grep -F "EnvironmentFile=/var/lib/fishystuff/gitops-beta/secrets/acme.env" "$unit" >/dev/null
grep -F "ExecStart=$(readlink -f "${mgmt_dir}/mgmt") run --tmp-prefix --no-pgp lang --converged-timeout -1 main.mcl" "$unit" >/dev/null
...
grep -F "ExecStart=$(readlink -f "${mgmt_dir}/mgmt") run --tmp-prefix --no-pgp lang --converged-timeout 600 main.mcl" "$stdout_unit" >/dev/null
...
expect_fail_contains \
  "refuse non-beta secrets env path" \
  "secrets_env_file must stay under /var/lib/fishystuff/gitops-beta/secrets/" \
  bash scripts/recipes/gitops-beta-tls-resident-unit.sh \
    - \
    /var/lib/fishystuff/gitops-beta/desired/beta-tls.staging.desired.json \
    "${mgmt_dir}/mgmt" \
    "$gitops_dir" \
    /var/lib/fishystuff/gitops/secrets/acme.env
