...
environment="$(normalize_named_arg environment "${4-production}")"
...
latest_proof_file() {
  local dir="$1"
  find "$dir" -maxdepth 1 -type f -name "${environment}-operator-proof.*.json" -printf '%T@ %p\n' \
    | sort -nr \
...
if [[ -z "$proof_file" ]]; then
  ...
  if [[ ! -d "$proof_dir" ]]; then
    echo "${environment} operator proof directory does not exist: ${proof_dir}" >&2
    exit 2
  fi
  proof_file="$(latest_proof_file "$proof_dir")"
  if [[ -z "$proof_file" ]]; then
    echo "no ${environment} operator proof artifact found in ${proof_dir}" >&2
    exit 2
  fi
...
if [[ ! -f "$proof_file" ]]; then
  echo "${environment} operator proof does not exist: ${proof_file}" >&2
  exit 2
fi
...
if ! jq -e \
  ...
  echo "${environment} operator proof does not record the required successful local checks" >&2
  exit 2
fi
...
if ! jq -e '
  ...
  | .commands.inventory.kv.gitops_host_inventory_ok == $environment
  and .commands.preflight.kv.gitops_preflight_ok == $draft
  and .commands.preflight.kv.handoff_summary == $summary
  and .commands.preflight.kv.admission_evidence == $admission
  and .commands.host_handoff_plan.kv.gitops_host_handoff_plan_ok == $draft
  and .commands.host_handoff_plan.kv.handoff_summary == $summary
  ' "$proof_file" >/dev/null; then
  echo "${environment} operator proof command outputs do not match proof inputs" >&2
  exit 2
fi
...
if [[ -z "$created_epoch" ]]; then
  echo "${environment} operator proof created_at is not parseable: ${created_at}" >&2
  exit 2
fi
now_epoch="$(date -u '+%s')"
age_seconds="$((now_epoch - created_epoch))"
if (( age_seconds < -300 )); then
  echo "${environment} operator proof created_at is in the future: ${created_at}" >&2
  exit 2
fi
if (( max_age_seconds > 0 && age_seconds > max_age_seconds )); then
  echo "${environment} operator proof is stale: age ${age_seconds}s exceeds max_age_seconds=${max_age_seconds}" >&2
  exit 2
fi
...
printf 'gitops_operator_proof_check_ok=%s\n' "$proof_file"
printf 'gitops_operator_proof_sha256=%s\n' "$proof_sha256"
printf 'gitops_operator_proof_environment=%s\n' "$(jq -er '.environment' "$proof_file")"
printf 'gitops_operator_proof_created_at=%s\n' "$created_at"
printf 'gitops_operator_proof_age_seconds=%s\n' "$age_seconds"
printf 'gitops_operator_proof_max_age_seconds=%s\n' "$max_age_seconds"
...
printf '%s_sha256=%s\n' "$proof_key_prefix" "$proof_sha256"
printf '%s_environment=%s\n' "$proof_key_prefix" "$(jq -er '.environment' "$proof_file")"
printf '%s_created_at=%s\n' "$proof_key_prefix" "$created_at"
printf '%s_age_seconds=%s\n' "$proof_key_prefix" "$age_seconds"
printf '%s_max_age_seconds=%s\n' "$proof_key_prefix" "$max_age_seconds"
