...
target_status() {
  ...
  if [[ "$value" != *@* ]]; then
    printf 'invalid_user_host'
    return
  fi
  user="${value%@*}"
  host="${value#*@}"
  if [[ "$user" != "root" ]]; then
    printf 'blocked_non_root_user'
    return
  fi
...
require_expected_hostname() {
  ...
  if [[ -z "$value" ]]; then
    echo "expected_hostname is required" >&2
    exit 2
  fi
  if [[ "$value" == *=* ]]; then
    echo "expected_hostname must be a hostname, got ${value}; pass arguments in order: target expected_hostname desired_state unit_file" >&2
    exit 2
  fi
  if [[ ! "$value" =~ ^[a-zA-Z0-9][a-zA-Z0-9.-]*$ ]]; then
    echo "expected_hostname must be a simple hostname, got: ${value}" >&2
    exit 2
  fi
}
...
target_check="$(target_status "$target")"

printf 'gitops_beta_remote_install_tls_resident_packet_ok=true\n'
printf 'beta_remote_tls_resident_install_packet_unit_name=%s\n' "$unit_name"
printf 'beta_remote_tls_resident_install_packet_target=%s\n' "${target:-<missing>}"
printf 'beta_remote_tls_resident_install_packet_target_status=%s\n' "$target_check"
printf 'beta_remote_tls_resident_install_packet_expected_hostname=%s\n' "$expected_hostname"
printf 'beta_remote_tls_resident_install_packet_desired_state=%s\n' "$desired_state"
printf 'beta_remote_tls_resident_install_packet_unit_file=%s\n' "$unit_file"
printf 'beta_remote_tls_resident_install_packet_cloudflare_token_source=%s\n' "${cloudflare_token_source:-<missing>}"
printf 'beta_remote_tls_resident_install_packet_active_secretspec_profile=%s\n' "$active_profile"
...
case "$active_profile" in
  production-deploy | prod-deploy | production)
    printf 'beta_remote_tls_resident_install_packet_status=blocked_profile\n'
    printf 'beta_remote_tls_resident_install_packet_next_required_action=load_beta_deploy_or_no_operator_profile\n'
    exit 0
    ;;
esac
...
if [[ ! -f "$desired_state_path" ]]; then
  printf 'beta_remote_tls_resident_install_packet_desired_state_status=missing\n'
  printf 'beta_remote_tls_resident_install_packet_next_command_01=just gitops-beta-tls-desired output=%s\n' "$desired_state"
  missing_inputs=1
else
  require_beta_tls_desired_shape "$desired_state_path"
  desired_sha256="$(sha256_file "$desired_state_path")"
  printf 'beta_remote_tls_resident_install_packet_desired_state_status=ready\n'
  printf 'beta_remote_tls_resident_install_packet_desired_sha256=%s\n' "$desired_sha256"
fi

if [[ ! -f "$unit_file_path" ]]; then
  printf 'beta_remote_tls_resident_install_packet_unit_file_status=missing\n'
  printf 'beta_remote_tls_resident_install_packet_next_command_02=just gitops-beta-tls-resident-unit output=%s\n' "$unit_file"
  missing_inputs=1
else
  require_beta_tls_unit_shape "$unit_file_path"
  unit_sha256="$(sha256_file "$unit_file_path")"
  printf 'beta_remote_tls_resident_install_packet_unit_file_status=ready\n'
  printf 'beta_remote_tls_resident_install_packet_unit_sha256=%s\n' "$unit_sha256"
fi

case "$cloudflare_token_source" in
  env:CLOUDFLARE_API_TOKEN)
    if [[ -z "${CLOUDFLARE_API_TOKEN:-}" ]]; then
      printf 'beta_remote_tls_resident_install_packet_cloudflare_token_status=missing_env\n'
      printf 'beta_remote_tls_resident_install_packet_next_required_action=load_beta_deploy_cloudflare_token\n'
      printf 'beta_remote_tls_resident_install_packet_next_command_03=secretspec run --profile beta-deploy -- just gitops-beta-remote-install-tls-resident-packet target=%s expected_hostname=%s desired_state=%s unit_file=%s cloudflare_token_source=env:CLOUDFLARE_API_TOKEN\n' "$target" "$expected_hostname" "$desired_state" "$unit_file"
      missing_inputs=1
    else
      token_sha256="$(sha256_line "$CLOUDFLARE_API_TOKEN")"
      printf 'beta_remote_tls_resident_install_packet_cloudflare_token_status=ready_env\n'
      printf 'beta_remote_tls_resident_install_packet_cloudflare_token_sha256=%s\n' "$token_sha256"
    fi
    ;;
  env:*)
    ...
    ;;
  "")
    printf 'beta_remote_tls_resident_install_packet_cloudflare_token_status=missing_source\n'
    printf 'beta_remote_tls_resident_install_packet_next_required_action=choose_cloudflare_token_source\n'
    missing_inputs=1
    ;;
  *)
    if [[ ! -f "$cloudflare_token_source_path" ]]; then
      printf 'beta_remote_tls_resident_install_packet_cloudflare_token_status=missing_file\n'
      printf 'beta_remote_tls_resident_install_packet_next_required_action=write_or_select_cloudflare_token_source_file\n'
      missing_inputs=1
    else
      token_sha256="$(sha256_file "$cloudflare_token_source_path")"
      printf 'beta_remote_tls_resident_install_packet_cloudflare_token_status=ready_file\n'
      printf 'beta_remote_tls_resident_install_packet_cloudflare_token_sha256=%s\n' "$token_sha256"
    fi
    ;;
esac

if [[ "$target_check" != "ready" ]]; then
  printf 'beta_remote_tls_resident_install_packet_status=blocked_target\n'
  printf 'beta_remote_tls_resident_install_packet_next_required_action=select_fresh_beta_ipv4_target\n'
  exit 0
fi

if [[ "$missing_inputs" != "0" ]]; then
  printf 'beta_remote_tls_resident_install_packet_status=pending_inputs\n'
  exit 0
fi

target_arg="$(kv_command_arg target "$target")"
expected_hostname_arg="$(kv_command_arg expected_hostname "$expected_hostname")"
